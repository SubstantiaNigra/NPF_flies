...
var_number = 10; % [0.3 0.15]
var_idx = indices{1, var_number}(1);
...
for i = var_idx
    if to_be_flipped == 1
        ...
        for j = 1:flies_nr
            valid1 = (~isempty(FD{1, i}.TrialN.Arena{j,2}) && FD{1, i}.TrialN.Arena{j,2} > nmin); % check left side
            valid2 = (~isempty(FD{1, i}.TrialN.Arena{j,1}) && FD{1, i}.TrialN.Arena{j,1} > nmin); % check right side of variant
        ...
        for k = 1:flies_nr
            sum_of_rewards(1, k) = sum(double(~isnan(FD{1, i}.TrigEvent.Arena{k,2})));
            sum_of_rewards(2, k) = sum(double(~isnan(FD{1, i}.TrigEvent.Arena{k,1})));
        end
...
for i = ivec(valid_flies(1,:))
    ...
    % -------------------------------------------
    if to_be_flipped
        nmax_1 = FD{1, var_idx}.TrialN.Arena{i,2}; % num of trials on left side
        nmax_2 = FD{1, var_idx}.TrialN.Arena{i,1}; % num of trials on right side
        clear X y
        Choices_1 = FD{1, var_idx}.Returns.Arena{i,2}(1,1:nmax_1);
        Choices_2 = FD{1, var_idx}.Returns.Arena{i,1}(1,1:nmax_2);
        Rewards_1 = ~isnan(FD{1, var_idx}.TrigEvent.Arena{i,2}(1:nmax_1));
        Rewards_2 = ~isnan(FD{1, var_idx}.TrigEvent.Arena{i,1}(1:nmax_2));
